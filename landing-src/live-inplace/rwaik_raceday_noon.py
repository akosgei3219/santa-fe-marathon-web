"""RWAIK page (4493): Kosgei ruled the race-day collection time — "at noon".

Two spots: the race-day row (which named Reunity with no time) and the pre-order note. Written as
12:00 PM, matching the Friday/Saturday rows' time style. Nothing else is implied: no window, no end
time, because only the hour was ruled. Noon sits inside the finish festival (9:00 AM - 1:00 PM).
usage: python rwaik_raceday_noon.py [--commit]
"""
...
EDITS = [
    ...
    ("pre-order note",
     'or at Reunity Resources Farm on race day.',
...
assert "Railyard" not in new and new.count("12:00 PM") == 3  # Friday row + the two new ones
print("GUARDS PASS")
if not COMMIT:
